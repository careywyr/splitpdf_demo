...
(function($) {
    ...
    function init(options) {
        ...
        return this.each(function() {
            ...
            if (!plugin) {
                plugin = new TouchPDF($("#myPDF"), options);

                //$this.data(PLUGIN_NS, plugin);
                $("#myPDF").data(PLUGIN_NS, plugin);
            }
        });
    }
     ...
     * @class
     */
    function TouchPDF(element, options1) {
        ...
        function initDom() {
            ...
            if (options.showToolbar) {
                ...
                $element.find(".pdf-toolbar").html(
                    ...
                    '<div class="pdf-button"><a class="pdf-prev"></a></div>' +
                    '<div class="pdf-button"><input type="number" id="pageNumber" class="toolbarField pageNumber" value="1" size="4" min="1" tabindex="15" max="14"><span class="pdf-page-count"></span></div>' +
                    '<div class="pdf-button"><a class="pdf-next"></a></div>' +
                    (options.disableZoom ? '' : '<div class="pdf-button"><a class="pdf-zoomin"></a></div>' +
                        '<div class="pdf-button"><a class="pdf-zoomout"></a></div>')
                );
                /*$('.pdf-next').css('background-image','url(../img/next.jpg) no-repeat')
                .css('background-size','30px 30px')
                .css('width','30px')
                .css('height','30px');
                $('.pdf-prev').css('background-image','url(../img/pre.jpg) no-repeat')
                .css('background-size','30px 30px')
                .css('width','30px')
                .css('height','30px');
                $('.pdf-zoomin').css('background-image','url(../img/add.jpg) no-repeat')
                .css('background-size','30px 30px')
                .css('width','30px')
                .css('height','30px');
                $('.pdf-zoomout').css('background-image','url(../img/redu.jpg) no-repeat')
                ...
                $element.find(".pdf-toolbar > .pdf-button > .pdf-prev").on("click", function() {
                    ...
                    //     source: "mm.pdf"
                    // });
                    // $(".pdf-toolbar").siblings().next().last().css('display', 'none');
                    /*$element.pdf("goto", 1);*/
                    /*goto(pageNum-1);*/
                });
            ...
            if (!options.disableSwipe) {
                $viewer.swipe({
                    swipe: function(event, direction, distance, duration, fingerCount, fingerData) {
                        ...
                        if (direction == "right") {
                            prevPage();
                        } else if (direction == "left") nextPage();
                    },
                    threshold: 50,
        ...
        function querypdfCount() {
            var totalPage = "";
            $.ajax({
                url: 'http://127.0.0.1/pdfview/PdfCountServlet',
                data: {},
                async: false,
                dataType: 'jsonp',
                jsonp: 'callbackd', //传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名(默认为:callback)
                jsonpCallback: "success_jsonpCallback", //自定义的jsonp回调函数名称，默认为jQuery自动生成的随机函数名
                contentType: 'application/json; charset=utf-8',
                success: function(data, status) {
                    if (status == 'success') {
                        totalPage = data[0].totalPage;
                        $(".pdf-page-count").html(" / " + totalPage);
                        return totalPage;
                    }
                },
        ...
        function renderPage() {
            ...
            pdfDoc.getPage(pageNumRendering).then(function(page) {
                ...
                $(".pdf-canvas")
                    /*.css("transform", "scale("+(1/options.quality+0.08)+")")*/
        ...
        function redraw() {
            ...
            $element.find(".pdf-outerdiv")
                ...
                .css("height", pdfHeight + TOOLBAR_HEIGHT*10 + BORDER_WIDTH*2)
        ...
        function updatePageCount() {
            if (state == EMPTY || state == INIT) return;
            totalPages = querypdfCount();
            $("#totalPage").val(totalPages);
            $element.find(".pdf-page-count").html( /*pageNum + */ " / " + totalPages);
            /* $element.find(".pageNumber").val(pageNum);*/
        }
        $('#pageNumber').bind('change', function(e) {
            turnToPage();
            // var pagesize = $element.find(".pageNumber").val();
            // var pagenum = parseInt(pagesize);
            // $element.pdf("goto", pagenum);
